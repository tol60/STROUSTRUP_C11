// g++ -std=c++11 -g -o ch10.e.10R.calculator_file_Rational.exe ch10.e.10R.calculator_file_Rational.C Roman.C Rational.C 2>&1 | tee tmp.txt
// This is a version of the calculator that will operate with Rational numbers (A/B + C/D = (AD+BC)/BC )
/* Created a version of the calculator with Rational numbers.
 * Roman numbers were disabled (as they interfered with the use of variables.
 */

/* Add a command "from x" to the calculator from Chapter 7 that makes it take input  from a file x.
  Example:
  ./ch10.e.10R.calculator_file_Rational.exe
...
struct Token {
  ...
  Rational value;
  string name;
  Token(char ch) :kind(ch), value((long int)0) { }                // Extra literals
  Token(char ch, Rational val) :kind(ch), value(val) { }  // Number
  Token(char ch, string s) :kind(ch), name(s),value((long int)0) { }       // initialize token for variable // problem01
};
...
Token Token_stream::get(istream& istr) // support the input from the file
{
  ...
  { istr.unget(); // put digit back into the input stream
    ...
    return Token(number,(Rational) val); // read a floating point number
  }
/*
  ...
  { istr.unget(); // put digit back into the input stream
    ...
    return Token(number,(long int) val.as_int()); // Roman integer, convert to int
  }
  */
  default:
    if (isalpha(ch)) {
...
struct Variable {
  ...
  Rational value;
  bool constant;
  Variable(string n, Rational v, bool constant = false) :name(n), value(v), constant(constant) { }
};
...
public:
  ...
  Rational get(string s);         // retrieve the value of user-defined variable (or constant)
  void set(string s, Rational d); // assign user-defined variable a value
  bool is_declared(string s);   // check if the variable has been declared (to avoid redefining a variable caused by a typo)
  Rational declare(string var, Rational val, bool constant=false); // define a new variable (constant if constant=true) and assign it a new value
...
Rational Symbol_table::get(string s) {
...
void Symbol_table::set(string s, Rational d) {
  for (int i = 0; i<var_table.size(); ++i)
...
Rational Symbol_table::declare(string name, Rational val, bool constant)
{
...
Rational primary()
{
  ...
  case name:  // handle Variables and functions
    if (t.name == "sqrt") { // sqrt ( Expression ) function
      ...
      Rational d = expression();
      double d_val = d.con_double();
      if (d_val < 0) error("Attempt to take sqrt of a negative number " + to_string(d));
      t = ts.get(); // expect ')'
      if (t.kind != ')') error("sqrt function call, expected ')'");
      return (Rational) sqrt(d_val); // convert from double to Ratioanal and return
      ...
      Rational w = expression ();
      //int i2 = narrow_cast<int> (w);  // cast to int
      t = ts.get(); // expect ')'
      if (t.kind != ')') error("pow function call, expected ')'");
      return (Rational) pow(d.con_double(),w.con_double());
    } else  // variable
...
Rational term()
{
  ...
  while(true) {
    ...
    case '/':
    { Rational d = primary();
      if (d.num() == 0) error("divide by zero");
      left /= d;
      break;
    }
    case '%':
    {
      Rational d = primary();
      if (d.num() == 0) error("divide by zero");
      left = (Rational)fmod(left.con_double(),d.con_double()); // x%y = x-y*int(x/y); 6.7%3.3=6.7-3.3*int(6.7/3.3) = 0.1
      break;
    }
    default:
...
Roman::Roman_int statementRoman(){
  int parm =  (int) (statement()).con_double();
...
void calculate()
{
  while(true) try {
    ...
    if (t.kind == help) {
      ...
      Rational answer = statement();
      ts.get_cout() << result << answer;
      string str = answer.whole_part_repr(); // this part was requred if we wanted to represent the whole part:
      if (str != "") ts.get_cout() << result << str;
      ts.get_cout() << endl;
