 ...
 * D8. Try reading as decimal, octal, hexadecimal, etc.:
 *     cin >> a >> oct >> b >> hex >> c >> d;
 *     cout << a << '\t' << b << '\t'<< c << '\t' << d << endl;
 */
// g++ -std=c++11 -g ch11.d.8_Input.C  -o ch11.d.8_Input.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  int a,b,c,d;
  cout << "Try to enter 1234 1234 1234 1234\n";
  cin >> a >> oct >> b >> hex >> c >> d;
  cout << a << '\t' << b << '\t'<< c << '\t' << d << endl;
  return 0;
}
