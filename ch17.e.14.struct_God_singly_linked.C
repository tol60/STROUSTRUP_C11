// g++ -w -Wall -std=c++11 ch17.e.14.struct_God_singly_linked.C -o ch17.e.14.struct_God_singly_linked.exe 2>&1 | tee tmp.txt
...
public:
  ...
  void  clear_ptrs(){succ=nullptr;} // in case we want to move a Link element from one list to another
...
Link* Link::add_ordered(Link* n) // insert n in the correct lexicographical position
{
  ...
  // otherwise we need to return this (after adding a new element down the chain)
  // at this point p->gd.name < nm
  // so we can check p->succ->gd.name

  while (p->succ) {
    if (p->succ->gd.name >= nm) // if lexicographically the new element should go before this one
    {
  ...
  // if we got here, it means we are at the end of the chain. Need to add an element here
  p->add(n); // important: as we advance in the loop, can not use this pointer here (need to use p->)
  return this;
}
...
Link* Link::advance(int n) const// move n positions in list; return nullptr for "not found"
    // positive n moves forward, negative = 0
{
  Link* p = (Link*)this;
  if (p==nullptr) return nullptr;
  if (n>0) {
