// g++ -std=c++11 -g -o ch7.e.3.calculator08buggy_fixed.exe ch7.e.3.calculator08buggy_fixed.C  2>&1 | tee tmp.txt
...
{
  Token t = ts.get();
  if (t.kind != 'a') error ("name expected in declaration_const");
  string name = t.name;
  // we can redeclare variable as a const
  // if (is_declared(name)) error(name, " declared twice");
  Token t2 = ts.get();
  ...
  double d = expression();
  // we can redeclare variable as a const
  if (is_declared(name)) { // the variable was already defined
    // find that element
    int indx=-1;
    for (int i = 0; i<names.size(); ++i){
    ...
    if (names[indx].name != name) error ("declaration_const: internal error");
    if (names[indx].constant) error ("declaration_const: trying to redefine previously defined constant");

    names[indx].constant = true; // redefine variable as a constant
    names[indx].value = d; // assign new value.

  } else { // brand new variable.
    names.push_back(Variable(name,d,true)); // Mark new variable as a constant
  }
  return d;
}
