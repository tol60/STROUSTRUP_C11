...
{  // add() the Points to this Shape
  for (Point p:lst)
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch13.7_ClosedPolyline.C \
           `fltk-config --ldflags --use-images` -o ch13.7_ClosedPolyline.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Simple_window win {pt, 600, 400,"ch13.7_ClosedPolyline"};  // make a simple window, {top_left, width, hight, title}
  ...
  /*
  Closed_polyline cpl_two ={{100,100},{200,200}};
  cpl_two.set_color(Color::red);
  cpl_two.set_style(Line_style(Line_style::dot,10));
  win.attach(cpl_two);
  */
