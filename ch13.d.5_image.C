...
5. Graph.h line 10: Uncomment #include "std_lib_facilities.h"
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch13.d.5_image.C \
           `fltk-config --ldflags --use-images` -o ch13.d.5_image.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Simple_window win {pt, 800, 1000,"ch13.d.5_image"};  // make a simple window, {top_left, width, hight, title}
