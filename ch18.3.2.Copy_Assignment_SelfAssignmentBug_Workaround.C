// g++ -g -w -Wall -std=c++11 ch18.3.2.Copy_Assignment_SelfAssignmentBug_Workaround.C -o ch18.3.2.Copy_Assignment_SelfAssignmentBug_Workaround.exe 2>&1 | tee tmp.txt
// Note:
// Deleting the old data before creating new ones could cause self-assignment problems
// This code is supposed to demonstrate the workaround
#include <iostream>
...
vector& vector::operator=(const vector& a) {
  ...
  if (this == &a)  {
    cout << "self-assignment detected, do nothing\n";
  } else {
    // make this vector a copy of vector a
    delete[] elem;  // deallocate old space before copying the data. Will it cause problems?
    double* p = new double[a.sz]; // allocate new space
    copy(a.elem,a.elem+a.sz,p);   // copy elements to newly allocated location
    // Original way: // delete[] elem;  // deallocate old space
