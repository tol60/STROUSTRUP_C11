...
Polygon needs explicit declaration similar to the Shape::Shape(initializer_list<Point> lst), as the Polygon::add(Point p) performs extra cheker
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch13.12_Circle.C \
           `fltk-config --ldflags --use-images` -o ch13.12_Circle.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Simple_window win {pt, 600, 400,"ch13.12_Circle"};  // make a simple window, {top_left, width, hight, title}
  ...
  Text t{Point{50,400},"Three circles, aligned with their centers in a horizontal line"};
