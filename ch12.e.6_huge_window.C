...
5. Graph.h line 10: Uncomment #include "std_lib_facilities.h"
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch12.e.6_huge_window.C \
           `fltk-config --ldflags --use-images` -o ch12.e.6_huge_window.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Simple_window win{tl, 2*x_max(), 2*y_max(), "Window wich is large then the scrren"};   // make a simple window, {top_left, width, hight, title}
