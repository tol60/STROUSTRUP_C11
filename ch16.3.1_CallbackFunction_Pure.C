...
/* ch16.3.1_CallbackFunction_Pure
...
https://github.com/thelastpolaris/Programming-Principles-and-Practice-Using-C-/tree/master/14
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -g -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch16.3.1_CallbackFunction_Pure.C \
           `fltk-config --ldflags --use-images` -o ch16.3.1_CallbackFunction_Pure.exe 2>&1 | tee tmp.txt
...
See ch16.3.1_CallbackFunction_Pure_debug_FLTK.C
...
int main ()
{
  ...
  constexpr Point pt{23,34};    // to become top left corner of window
  ...
  //Simple_window win {pt, xmax, ymax,"ch16.3.1_CallbackFunction_Pure"};  // make a simple window, {top_left, width, height, title}
  ...
  //Graph_lib::Window win {pt, xmax, ymax, "ch16.3.1_CallbackFunction_Pure"};  // make a simple window, {top_left, width, height, title}
