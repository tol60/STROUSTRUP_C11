// Find the mode = the string that appears most times
// g++ -std=c++11 -g -o ch4.e.17_strings_mode.exe ch4.e.17_strings_mode.C  2>&1 | tee tmp.txt

#include "std_lib_facilities.h"

int main ()
{
  vector <string> temps; // temperatures
  cout << "Please enter a bunch of strings:\n";
  for (string temp; cin >> temp;) // read into temp
    temps.push_back(temp);  // put temp into vector

  sort (temps);
  cout << "The smallest string is " <<temps[0] << endl;  // as vector is sorted, should give min
  cout << "The largest string is " <<temps[temps.size()-1] << endl; // as vector is sorted, should give max

  string mode{temps[0]};  // we assume this is the mode
  ...
  cout << "The mode (the string that appears most times) is \"" << mode << "\" and it appeared " << mode_count << " times\n";
  return 0;
}
