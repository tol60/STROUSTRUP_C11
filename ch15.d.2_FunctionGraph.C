...
https://github.com/thelastpolaris/Programming-Principles-and-Practice-Using-C-/tree/master/14
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -g -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch15.d.2_FunctionGraph.C \
           `fltk-config --ldflags --use-images` -o ch15.d.2_FunctionGraph.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  //Simple_window win {pt, 600, 400,"ch15.d.2_FunctionGraph"};  // make a simple window, {top_left, width, height, title}
  Graph_lib::Window win {pt, xmax, ymax, "ch15.d.2_FunctionGraph"};  // make a simple window, {top_left, width, height, title}
