...
Polygon needs explicit declaration similar to the Shape::Shape(initializer_list<Point> lst), as the Polygon::add(Point p) performs extra cheker
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch13.e.10_Regular_polygon.C \
           `fltk-config --ldflags --use-images` -o ch13.e.10_Regular_polygon.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Point pt{100,100};    // a variable point

  Simple_window win {pt, 600, 400,"ch13.e.10_Regular_polygon"};  // make a simple window, {top_left, width, hight, title}
  //int radius = 70; // radius to feed to Regualr_hexagon constructor
  constexpr int radius = 40;

  Polygon pgon;

  int num_sides = 3; // triangle
  Regular_polygon tri(pt,radius, num_sides);
  win.attach(tri);

  pt += Point{100,0};
  num_sides = 4; // square
  Regular_polygon sqr(pt,radius, num_sides);
  sqr.set_fill_color(Color::yellow);
  win.attach(sqr);

  pt += Point{100,0};
  num_sides = 5; // pentagon
  Regular_polygon pen(pt,radius, num_sides,45);
  win.attach(pen);
