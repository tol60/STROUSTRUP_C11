 ...
 */
// g++ -std=c++11 -g ch11.3.2_BinaryFile_Present.C  -o ch11.3.2_BinaryFile_Present.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  for (int x; ifs.read((char *) &x,sizeof(int)); ) // note: reading bytes
    v.push_back(x);

  // .. do something with v ..

  for (int x:v)
    ofs.write((char *) &x,sizeof(int)); // note: writing bytes
