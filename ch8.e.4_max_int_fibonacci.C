// Ex4. An int can hold integers only up ot a maximum number. Find an approximantion of that maximum number by using fibonacci().
...
//    2. a vector
// g++ -std=c++11 -g -o ch8.e.4_max_int_fibonacci.exe ch8.e.4_max_int_fibonacci.C  2>&1 | tee tmp.txt
