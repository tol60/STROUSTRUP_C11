...
Polygon needs explicit declaration similar to the Shape::Shape(initializer_list<Point> lst), as the Polygon::add(Point p) performs extra cheker
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch13.16_Mark.C \
           `fltk-config --ldflags --use-images` -o ch13.16_Mark.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Simple_window win {pt, 600, 400,"ch13.16_Mark"};  // make a simple window, {top_left, width, hight, title}
  ...
  Text t{Point{50,390},"Three circles, aligned with their centers in a horizontal line"};
  ...
  // trying to set a corresponding color
  m1.set_color(c1.color());
  m2.set_color(c2.color());
  m3.set_color(c3.color());
  ...
  //m3.add(Point{250,200});  // I think we inherit it all the way from Open_polyline (which is effectively Shape::add();
