...
Polygon needs explicit declaration similar to the Shape::Shape(initializer_list<Point> lst), as the Polygon::add(Point p) performs extra cheker
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch13.14_Marked_polyline.C \
           `fltk-config --ldflags --use-images` -o ch13.14_Marked_polyline.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Simple_window win {pt, 600, 400,"ch13.14_Marked_polyline"};  // make a simple window, {top_left, width, hight, title}
  ...
  */
  ...
  /*
  // '1' will show up twice
  Marked_polyline mpl{"123",{
    Point{100,100},
    Point{150,200},
    Point{250,250},
    Point{300,200}
  ...
  */
  ...
  /*
  // will mark the marks with default '*'
  Marked_polyline mpl{"",{
    Point{100,100},
    Point{150,200},
    Point{250,250},
