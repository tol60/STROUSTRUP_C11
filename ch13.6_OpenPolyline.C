...
{  // add() the Points to this Shape
  for (Point p:lst)
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch13.6_OpenPolyline.C \
           `fltk-config --ldflags --use-images` -o ch13.6_OpenPolyline.exe 2>&1 | tee tmp.txt
