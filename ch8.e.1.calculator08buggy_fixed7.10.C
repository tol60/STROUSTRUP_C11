// g++ -std=c++11 -g -o ch8.e.1.calculator08buggy_fixed7.10.exe ch8.e.1.calculator08buggy_fixed7.10.C  2>&1 | tee tmp.txt
...
/* Change log
  ...
  E10. Modify the calculator to operate on ints (only); give errors for overflow and underflow. Hint: Use narrow_cast
  // ch7.e.10.calculator08buggy_fixed.C

  ch8.e.1 Modify the calculator program from Chapter 7 to make the input stream
  an explicit parameter (as shown in 8.5.8), rather than simply using cin.
  Also give the Token_stream constructor (7.8.2) an istream& parameter so that when we figure out how to make our own istreams
  (e.g., attached to files), we can use the calculator for those.
...
public:
  ...
  Token_stream(istream& istr) :full(0), buffer(0) { } // Note: we need to finish this one. ZZZ10
...
// Token_stream ts; // ch8.e.1 made it a local variable (from global)

double expression(Token_stream& ts); // ch8.e.1
//double expression();

double primary(Token_stream& ts) // ch8.e.1
//double primary()
{
  Token t = ts.get();
  switch (t.kind) {
  ...
  case '-':
    return - primary(ts);
  case '+':
    return primary(ts); // problem06 did not handle + sign +2-3;
...
double term(Token_stream& ts) // ch8.e.1
//double term()
{
...
double declaration(Token_stream& ts) // ch8.e.1
//double declaration()
// "let" var_name "=" Expression
{
  Token t = ts.get();
...
double declaration_const(Token_stream& ts) // ch8.e.1
...
double statement(Token_stream& ts) // ch8.e.1
//double statement()
{
  Token t = ts.get();
  switch(t.kind) {
...
void calculate(Token_stream& ts) // ch8.e.1
//void calculate()
{
  while(true) try {
    cout << prompt;
