...
Polygon needs explicit declaration similar to the Shape::Shape(initializer_list<Point> lst), as the Polygon::add(Point p) performs extra cheker
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch13.9_Rectangle_3_put_on_top.C \
           `fltk-config --ldflags --use-images` -o ch13.9_Rectangle_3_put_on_top.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Simple_window win {pt, 600, 400,"ch13.9_Rectangle_3_put_on_top"};  // make a simple window, {top_left, width, hight, title}
  ...
  rect11.move(400,0); // to the right of rect 21
