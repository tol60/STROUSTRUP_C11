...
https://www.tutorialspoint.com/computer_graphics/polygon_filling_algorithm.htm
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch14.e.8_Octagon.C \
           `fltk-config --ldflags --use-images` -o ch14.e.8_Octagon.exe 2>&1 | tee tmp.txt
...
int main ()
{
  ...
  Simple_window win {pt, 600, 400,"ch14.e.8_Octagon"};  // make a simple window, {top_left, width, hight, title}
  ...
  Regular_Octagon roct{centA,radius,alpha};
  roct.set_fill_color(Color::blue);
  roct.set_color(Color::red);
  roct.move(100,50);
  win.attach(roct);
