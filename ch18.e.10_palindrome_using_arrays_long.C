// g++ -g -w -Wall -std=c++11 ch18.e.10_palindrome_using_arrays_long.C -o ch18.e.10_palindrome_using_arrays_long.exe 2>&1 | tee tmp.txt
 ...
 * 10. Look at the "array solution" to the palindrome problem in $18.7.2. Fix it to deal
 *    with long strings by (a) reporting if an input string was too long and
 *    (b) allowing an arbitrary long string. Comment on the complexity of the two versions.
 *
 */
...
bool is_palindrome( const char s[], int n) {
  ...
  while (first<last){ // we have not reached the middle
...
// (b) allowing an arbitrary long string. Comment on the complexity of the two versions.
istream& read_long_word(istream& is, char*& buffer, int& max){
  // read an arbitrary long word
  // we will potentially extend the buffer
  // as we resize the buffer (in read_long_word), char* buffer needs to be allocated on the free store
  char ch = '\n';
  int indx = 0; // current position in the buffer
  //while (is>> ch){
  while ( (ch=getchar())!= EOF ){
    if (ch==' '|| ch=='\t' || ch =='\n'){ // detected end of the word
      buffer[indx]=0;
      return is;
    } else {
      buffer[indx] = ch;
    ...
    ++indx; // prepare for next iteration

    // check if we need to resize the buffer
    if (indx == max) { // valid indexes [0,..max-1](and we already put a value). Now we need to resize
      char* new_buff = new char[2*max];
      for (int i=0; i<max; ++i)
        new_buff[i] = buffer[i]; // preserve the old value
...
int main()
{
  ...
  // (b) allowing an arbitrary long string.
  // as we resize the buffer (in read_long_word), char* buffer needs to be allocated on the free store
  char* s = new char[max];
  for (; read_long_word(cin,s,max);){
    // solution (a) reporting if an input string was too long
    if (str_len(s) +1 >= max) {
      cout << "Warning: input was too long. It will be split into the smaller pieces\n";
    }

    cout <<s<<" is";
    if(!is_palindrome(s,str_len(s))) cout << " not";
