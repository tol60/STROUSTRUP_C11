...
/* ch15.5_Approximation_debug_FLTK
...
https://github.com/thelastpolaris/Programming-Principles-and-Practice-Using-C-/tree/master/14
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -g -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch15.5_Approximation_debug_FLTK.C \
           `fltk-config --ldflags --use-images` -o ch15.5_Approximation_debug_FLTK.exe 2>&1 | tee tmp.txt
...
int int_fac(int n) // factorial(n); n!
{
  int r=1;
  while (n>1){
    r*=n;
...
double term(double x, int n) { return pow(x,n)/fac(n);} // n'th term of series

double expe(double x, int n) // some of n terms for x
{
  double sum = 0.0;
...
int main ()
{
  ...
  //constexpr int r_max = 10; // exponent was not depicted for some reason
  //constexpr int r_max = 2; // exponent was depicted fine
  //constexpr int r_max = 7; // exponent goes really high, no vertical bars. OK
  //constexpr int r_max = 8;  // displays one vertical bar
  //constexpr int r_max = 9; // displays vertical bars
  constexpr int r_max = 10; // exponent is not depicted at all. Problem!!!
  ...
  Simple_window win {pt, xmax, ymax,"ch15.5_Approximation_debug_FLTK"};  // make a simple window, {top_left, width, height, title}
  //Graph_lib::Window win {pt, xmax, ymax, "ch15.5_Approximation_debug_FLTK"};  // make a simple window, {top_left, width, height, title}
  ...
  cout<<"Trying to test exp(1.0) = " << exp(1.0) << endl;
  cout<<"r_max = " << r_max << endl;
  cout<<"Trying to test exp(r_max) = " << exp(r_max) << endl;
  cout<<"Trying to test int(exp(r_max)) = " << int(exp(r_max)) << endl;
  cout<<"y_scale = " << y_scale <<endl;
  cout<<"Trying to test y_scale*exp(r_max) = " << y_scale*exp(r_max) << endl;
  cout<<"Trying to test int(y_scale*exp(r_max)) = " << int(y_scale*exp(r_max)) << endl;
  cout<<"size(int) = " << sizeof(int) << endl;
  cout<<"size(long int) = " << sizeof(long int) << endl;

  // Debug lines
  Point deb_pt_1{300,280};
  Point deb_pt_2{340,153};
  Point deb_pt_3{380,-791};
  Point deb_pt_4{420,-7768};
  Point deb_pt_5{460,-59319}; // Problem
  // Point deb_pt_5_A{460,-40319}; // Problem
  // Point deb_pt_5_A{460,-20319}; // No problem
  // Point deb_pt_5_A{460,-30319}; // No problem
  // Point deb_pt_5_A{460,-35000}; // Problem
  // Point deb_pt_5_A{460,-32768}; // = 32k No problem // Note: FLTK/fltk-1.3.3/src/fl_rect.cxx #define SHRT_MAX (32767)
  // Point deb_pt_5_A{460,-34000}; // Problem
  // Point deb_pt_5_A{460,-33000}; // Problem
  // Point deb_pt_5_A{460,-32900}; // Problem
  // Point deb_pt_5_A{460,-32800}; // No problem
  // Point deb_pt_5_A{460,-32850}; // No problem
  // Point deb_pt_5_A{460,-32875}; // No problem
  // Point deb_pt_5_A{460,-32893}; // No problem
  // Point deb_pt_5_A{460,-32899}; // Problem
  // Point deb_pt_5_A{460,-32896}; // No problem
   Point deb_pt_5_A{460,-32898}; // Problem // with Point deb_pt_4{420,-7768};
  // Point deb_pt_5_A{460,-32897}; // No problem // with Point deb_pt_4{420,-7768};

   //Point deb_pt_5_A_X{460,-49152}; // = 32k*1.5 // Problem, gows down
   //Point deb_pt_5_A_X{460,-65536}; // = 32k*2 // goes up
   // Point deb_pt_5_A_X{460,-65536+400}; // = 32k*2 - 400 // goes down, contained on the screen
   // Point deb_pt_5_A_X{460,-65536+300+130}; // = 32k*2 - 430// goes down, contained on the screen
   // Point deb_pt_5_A_X{460,-65536+280}; // = 32k*2 - 280// goes down, contained on the screen
   Point deb_pt_5_A_X{460,-65536+300}; // = 32k*2 - 300// goes down, contained on the screen
 ...
 65   where LW = current line width for drawing. This is done so that
  ...
  Point deb_pt_4_B{420,-7767};


  Line deb_line_1{deb_pt_1, deb_pt_2};
  Line deb_line_2{deb_pt_2, deb_pt_3};
  Line deb_line_3{deb_pt_3, deb_pt_4};
  Line deb_line_4{deb_pt_4, deb_pt_5}; // this line causes problem (shows up at the bottom of the screen)
  Line deb_line_4_A{deb_pt_4, deb_pt_5_A};
  Line deb_line_4_B{deb_pt_4_B, deb_pt_5_A};

  Line deb_line_25A{deb_pt_2, deb_pt_5_A};
  Line deb_line_25A_X{deb_pt_2, deb_pt_5_A_X};

  deb_line_2.set_color(Color::dark_green);
  deb_line_3.set_color(Color::dark_yellow);
  deb_line_4.set_color(Color::red);

  deb_line_4_B.set_color(Color::dark_cyan);


  win.attach(deb_line_1);
  /*
  win.attach(deb_line_2);
  win.attach(deb_line_3);
  */
  // //win.attach(deb_line_4);// this line causes problem (shows up at the bottom of the screen)
  // win.attach(deb_line_4_A); // manual debug
  // //win.attach(deb_line_4_B); // manual debug

  //win.attach(deb_line_25A); // this causes a problem - instead of going up, going down
  win.attach(deb_line_25A_X); // new experiment
