//
/* EX08: Use the program from the previous exercise to make a dictionary (as an alternative to the apprach in $11.7)
...
{
  vector <string> vs; // will temporary store all the words before removing the doubles.
  ...
  while (getline(inFile,line)) { // when we read the line, it is missing '\n' = endl now
    ...
    // Ex08: we create a dictionary instead
  ...
  cout << "The dictionary contains " << cnt << " words" << endl;
