// Ex 16: Define an input operator (>>) that reads monetary amounts with currency denominations,
 ...
 */
// g++ -std=c++11 -g ch9.e.16_MoneyClassCurrencyInput.C  -o ch9.e.16_MoneyClassCurrencyInput.exe 2>&1 | tee tmp.txt
...
public:
  ...
  Money (const Currency& cur, double cent);  // Accept a floating-point initializer as long as it can be exactly represented as a long int.
...
Money::Money (const Currency& cur, double cent)  // Accept a floating-point initializer as long as it can be exactly represented as a long int.
:mCurrency{cur},
...
istream& operator>>(istream& is, Money& m){
  ...
  string str;
  is >> ch;
  bool neg_value = false; // keep track of hte sign.
  if (ch == '-') {
    neg_value = true; // support for negative values (like -$34.5
    is >> ch;
  }
  while (isalpha(ch) || ch == '$'){
    str += ch;
  ...
  // see if the value matched one of our currency code.
  for (int i=0; i< currency_tbl.size(); ++i){
    if (currency_tbl[i]==str) {
      // we found the match in the currency.
      double val;
      is >> val;
      val *= 100.0; // adjust to format that the constructor understands
      if (neg_value) {
        val = -val;
      }
      m= Money(Currency(i),val);
      // m = Money(Currency::usd,10.0)
      // m = Money(Currency::usd, (long int)10 );
      return is;
    }
  }
  error("Did not recognize money symbol:", str);
  return is;
}

// output operator for Money
