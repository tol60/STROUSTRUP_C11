...
https://github.com/thelastpolaris/Programming-Principles-and-Practice-Using-C-/tree/master/14
   ...
   //g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp MY_CPP_FILE.cpp `fltk-config --ldflags --use-images` -o MY_EXECUTABLE
g++ -g -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp ch15.5_Approximation.C \
           `fltk-config --ldflags --use-images` -o ch15.5_Approximation.exe 2>&1 | tee tmp.txt
...
See ch15.5_Approximation_debug_FLTK.C
...
int main ()
{
  ...
  //constexpr int n_points = 10;  // debug was performed on this value
  ...
  Simple_window win {pt, xmax, ymax,"ch15.5_Approximation"};  // make a simple window, {top_left, width, height, title}
  //Graph_lib::Window win {pt, xmax, ymax, "ch15.5_Approximation"};  // make a simple window, {top_left, width, height, title}
  ...
  // Point deb_pt_5_A{460,-32768}; // = 32k No problem
